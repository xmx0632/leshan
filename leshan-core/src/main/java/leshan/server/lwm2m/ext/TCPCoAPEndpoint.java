 ...
 */
public class TCPCoAPEndpoint implements Endpoint {
	
	/** the logger. */
	private final static Logger LOGGER = Logger.getLogger(TCPCoAPEndpoint.class.getCanonicalName());
	 ...
	 * @param port the port
	 */
	public TCPCoAPEndpoint(int port) {
	 ...
	 */
	public TCPCoAPEndpoint(InetSocketAddress address) {
		this(address, NetworkConfig.getStandard());
	}
	
	public TCPCoAPEndpoint(NetworkConfig config) {
	 ...
	 */
	public TCPCoAPEndpoint(InetSocketAddress address, NetworkConfig config) {
		this(createTCPConnector(address, config), config);
	}
	 ...
	 */
	public TCPCoAPEndpoint(Connector connector, NetworkConfig config) {
		this.config = config;
	 ...
	 */
	private static Connector createTCPConnector(InetSocketAddress address, NetworkConfig config) {
		TCPConnector c = new TCPConnector(address);
		...
		private void receiveMessage(RawData raw) {
			...
			if (parser.isRequest()) {
				...
				if (!request.isCanceled()) {
					...
					if (exchange != null) {
						exchange.setEndpoint(TCPCoAPEndpoint.this);
