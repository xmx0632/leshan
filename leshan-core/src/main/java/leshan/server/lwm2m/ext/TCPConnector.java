 ...
 * {@link RawDataChannel#receiveData(RawData)}.
...
public class TCPConnector implements Connector {
	...
	public synchronized void start() throws IOException {
		...
		LOGGER.config("TCP-connector starts "+senderCount+" sender threads and "+receiverCount+" receiver threads");
		
		receiverThreads = new LinkedList<Thread>();
		for (int i=0;i<receiverCount;i++) {
			receiverThreads.add(new Receiver("TCP-Receiver "+localAddr+"["+i+"]"));
		}
		
		senderThreads = new LinkedList<Thread>();
		for (int i=0;i<senderCount;i++) {
			senderThreads.add(new Sender("TCP-Sender "+localAddr+"["+i+"]"));
		}
		...
		LOGGER.config("TCP connector listening on "+socket.getLocalSocketAddress()+", recv buf = "+receiveBuffer+", send buf = "+sendBuffer
